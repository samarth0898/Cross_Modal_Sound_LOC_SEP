...
class NetWrapper(torch.nn.Module):
    ...
    def forward(self, batch_data, args):
        ...
        B = mag_mix.size(0)
        T = mag_mix.size(3)
        print('Batch Size {}, Number of frames {} , Time {}'.format(B,N,T))
...
def output_visuals(vis_rows, batch_data, outputs, args):
    ...
    for j in range(B):
        ...
        wavfile.write(os.path.join(args.vis, filename_mixwav), 44100, mix_wav)
        ...
        for n in range(N):
            ...
            wavfile.write(os.path.join(args.vis, filename_gtwav), 44100, gt_wav)
            wavfile.write(os.path.join(args.vis, filename_predwav), 44100, preds_wav[n])
